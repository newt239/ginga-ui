...
- **`packages/components/*/`**: 個別のコンポーネントパッケージ（accordion、button、cardなど）
  - 各コンポーネントは独立したワークスペースパッケージ（例: `@ginga-ui/button`）
  - アクセシビリティのためReact Aria Componentsを使用
  - 各コンポーネントには専用のCSSファイルがエクスポートされる
- **`packages/utils/`**: AI テーマ生成クライアントを含む共有ユーティリティ
- **`stories/`**: コンポーネントドキュメント用のStorybookストーリー
- **`docs/`**: ドキュメントウェブサイト

### パスエイリアス
TypeScriptパスマッピングを使用:
```typescript
"@ginga-ui/*": ["./packages/*/src", "./packages/components/*/src"]
```
